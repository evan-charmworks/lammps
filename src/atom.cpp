  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (values[0] == nullptr)
      error->all(FLERR,"Incorrect atom format in data file");
    for (m = 1; m < nwords; m++) {
      values[m] = utils::strtok_r(nullptr," \t\n\r\f",&r_token);
      if (values[m] == nullptr)
        error->all(FLERR,"Incorrect atom format in data file");
    }
...
void Atom::data_vels(int n, char *buf, tagint id_offset)
{
  ...
  *next = '\n';
  ...
  for (int i = 0; i < n; i++) {
    ...
    values[0] = utils::strtok_r(r_token," \t\n\r\f",&r_token);
    for (j = 1; j < nwords; j++)
      values[j] = utils::strtok_r(nullptr," \t\n\r\f",&r_token);
...
void Atom::data_bodies(int n, char *buf, AtomVec *avec_body, tagint id_offset)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    else tagdata = ATOTAGINT(utils::strtok_r(nullptr," \t\n\r\f",&r_token)) + id_offset;
    ...
    ninteger = utils::inumeric(FLERR,utils::strtok_r(nullptr," \t\n\r\f",&r_token),false,lmp);
    ndouble = utils::inumeric(FLERR,utils::strtok_r(nullptr," \t\n\r\f",&r_token),false,lmp);
    ...
    if ((m = map(tagdata)) >= 0) {
      ...
      for (j = 0; j < ninteger; j++)
        ivalues[j] = utils::inumeric(FLERR,utils::strtok_r(nullptr," \t\n\r\f",&r_token),false,lmp);
      for (j = 0; j < ndouble; j++)
        dvalues[j] = utils::numeric(FLERR,utils::strtok_r(nullptr," \t\n\r\f",&r_token),false,lmp);
