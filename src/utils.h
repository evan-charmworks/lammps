...
namespace LAMMPS_NS {
  ...
  namespace utils {
    ...
    /** Tokenize a C string in a reentrant manner
     *
     *  \param str     string to be tokenized on first call, nullptr thereafter
     *  \param delim   set of characters that delimit the tokens
     *  \param saveptr parsing state, to be maintained between calls
     *  \return pointer to next token, or nullptr if none remaining
     */
